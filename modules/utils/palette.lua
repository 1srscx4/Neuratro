...
local function live_boss_hue()
	if not (G and G.C and G.C.DYN_UI and G.C.DYN_UI.BOSS_MAIN) then
		return nil
	end
	if not (G.GAME and G.GAME.blind and G.GAME.blind.boss) then
		return nil
	end
...
local function evil_boss_override()
	...
	if bb then
		ov.BLIND = { Boss = recolor_to_hue(bb, bh) }
	end
	return ov
end
...
local function apply_color_inplace(target, source)
	...
	target[2] = source[2]
	target[3] = source[3]
	if source[4] ~= nil then
		target[4] = source[4]
	end
end

local function snapshot_baseline()
	if _palette_baseline or not (G and G.C) then
		return
	end
...
local function apply_gc_table(palette)
	if not palette or not G or not G.C or not _palette_baseline then
		return
	end
	for _, k in ipairs(REPALETTE_KEYS) do
		...
		if G.C[tbl] then
			for _, sk in ipairs(subs) do
				local src = (palette[tbl] and palette[tbl][sk])
					or (
						_palette_baseline.nested
						and _palette_baseline.nested[tbl]
						and _palette_baseline.nested[tbl][sk]
					)
...
local function apply_partial_gc(palette)
	if not palette or not G or not G.C then
		return
	end
	for _, k in ipairs(REPALETTE_KEYS) do
		if palette[k] and G.C[k] then
			apply_color_inplace(G.C[k], palette[k])
		end
	end
	for tbl, subs in pairs(NESTED_KEYS) do
...
love.draw = function(...)
	if G and G.C then
		...
		if is_boss then
			local ov = (key == "neuro" and neuro_boss_override()) or (key == "evil" and evil_boss_override())
			if ov then
				apply_partial_gc(ov)
			end
		end
...
if love.filesystem.getInfo(_PREF_FILE) then
	...
	if data then
		_saved_pref = data:match("^%s*(.-)%s*$")
	end
end
if _saved_pref then
	AKYRS.config.palette = _saved_pref
end
